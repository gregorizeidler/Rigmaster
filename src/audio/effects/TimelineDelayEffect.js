/*
  ...
  - Reverse: real reverse via ReverseBufferProcessor (ring buffer + inverted playback); post-FX bus
  - Duck: envelope follower (ConstantSource + gain into delayMix.gain), no worklet
  - Lo-Fi/DBucket: BitcrusherProcessor (bit depth + downsample) on post-FX bus + grit/filter per mode
  ...
  NOTE: Requires PitchShifterGrain (BigSky), ReverseBufferProcessorCode, BitcrusherProcessorCode (TimelineWorklets).
...
import { ReverseBufferProcessorCode, BitcrusherProcessorCode } from './TimelineWorklets';
...
class TimelineDelayEffect extends BaseEffect {
  ...
  static async initWorklets(ctx) {
    ...
    await registerWorkletFromString(ctx, 'reverse-buffer', ReverseBufferProcessorCode);
    await registerWorkletFromString(ctx, 'bitcrusher-sr', BitcrusherProcessorCode);
  ...
  constructor(audioContext, id){
    ...
    this.iceToneL = ctx.createBiquadFilter();
    this.iceToneR = ctx.createBiquadFilter();
    this.iceToneL.type = 'lowpass';
    this.iceToneR.type = 'lowpass';
    this.iceToneL.frequency.value = 9000;
    this.iceToneR.frequency.value = 9000;
    this.iceAmtL = ctx.createGain();
    this.iceAmtR = ctx.createGain();
    this.iceAmtL.gain.value = 0;
    this.iceAmtR.gain.value = 0;
    ...
    this.duckDetLP = ctx.createBiquadFilter();
    this.duckDetLP.type = 'lowpass';
    this.duckDetLP.frequency.value = 30;
    this.duckRect = this.makeRectifier(ctx);
    this.duckBase = ctx.createConstantSource();
    this.duckBase.offset.value = 0;
    this.duckBase.start();
    this.duckScale = ctx.createGain();
    this.duckScale.gain.value = 0;
    this._duckConnected = false;
    ...
    this.postFxIn = ctx.createGain();
    this.postFxOut = ctx.createGain();
    this._postFxPatched = false;
    this.reverseNode = null;
    this.bitcrushNode = null;
    this.initExtraWorkletNodes();
  ...
  async initIceWorkletNodes() {
    ...
    try {
      this.iceUpL = new AudioWorkletNode(ctx, 'pitch-shifter-grain', { numberOfInputs: 1, numberOfOutputs: 1, outputChannelCount: [1] });
      this.iceUpR = new AudioWorkletNode(ctx, 'pitch-shifter-grain', { numberOfInputs: 1, numberOfOutputs: 1, outputChannelCount: [1] });
      this.iceUpL.parameters.get('semitones').value = 12;
      this.iceUpR.parameters.get('semitones').value = 7;
      this.iceUpL.parameters.get('grainSizeMs').value = 50;
      this.iceUpR.parameters.get('grainSizeMs').value = 60;
      this.iceUpL.parameters.get('mix').value = 1;
      this.iceUpR.parameters.get('mix').value = 1;

      this.tremGainL.connect(this.iceUpL);
      this.iceUpL.connect(this.iceToneL);
      this.iceToneL.connect(this.iceAmtL);
      this.iceAmtL.connect(this.delayL);

      this.tremGainR.connect(this.iceUpR);
      this.iceUpR.connect(this.iceToneR);
      this.iceToneR.connect(this.iceAmtR);
      this.iceAmtR.connect(this.delayR);
    } catch (e) {
      console.warn('Failed to create/connect Timeline ICE worklet nodes:', e);
    }
  }

  async initExtraWorkletNodes() {
    if (!TimelineDelayEffect.workletsInitialized) {
      await TimelineDelayEffect.initWorklets(this.audioContext);
    }
    const ctx = this.audioContext;
    try {
      this.reverseNode = new AudioWorkletNode(ctx, 'reverse-buffer', {
        numberOfInputs: 1,
        numberOfOutputs: 1,
        outputChannelCount: [2]
      });
      this.reverseNode.parameters.get('windowMs').value = 500;
      this.reverseNode.parameters.get('mix').value = 1.0;

      this.bitcrushNode = new AudioWorkletNode(ctx, 'bitcrusher-sr', {
        numberOfInputs: 1,
        numberOfOutputs: 1,
        outputChannelCount: [2]
      });
      this.bitcrushNode.parameters.get('bitDepth').value = 12;
      this.bitcrushNode.parameters.get('downsample').value = 1;
      this.bitcrushNode.parameters.get('mix').value = 1.0;
    } catch (e) {
      console.warn('Failed to create Timeline reverse/bitcrusher worklet nodes:', e);
    }
  }

  _unpatchPostFx() {
    if (!this._postFxPatched) return;
    try {
      this.postFxIn.disconnect();
      if (this.reverseNode) this.reverseNode.disconnect();
      if (this.bitcrushNode) this.bitcrushNode.disconnect();
    } catch (e) {}
    this.postFxIn.connect(this.postFxOut);
    this._postFxPatched = false;
  }

  _patchReverse() {
    if (!this.reverseNode) return;
    this._unpatchPostFx();
    try { this.postFxIn.disconnect(); } catch (e) {}
    this.postFxIn.connect(this.reverseNode);
    this.reverseNode.connect(this.postFxOut);
    this._postFxPatched = true;
  }
  ...
  _patchBitcrush() {
    ...
    this._unpatchPostFx();
    try { this.postFxIn.disconnect(); } catch (e) {}
    this.postFxIn.connect(this.bitcrushNode);
    this.bitcrushNode.connect(this.postFxOut);
    this._postFxPatched = true;
  }
  ...
  setupRouting(){
    ...
    this.msMerge.connect(this.postFxIn);
    this.postFxIn.connect(this.postFxOut);
    this.postFxOut.connect(this.delayMix);
  ...
  setDelayType(type){ const now=this.audioContext.currentTime; this.currentType=type;
    ...
    // Reset: leave duck path when not duck
    if (this._duckConnected) {
      try { this.duckBase.disconnect(); this.duckScale.disconnect(); } catch(e) {}
      this._duckConnected = false;
      this.delayMix.gain.setTargetAtTime(0.5, now, 0.02);
    }
    this.duckBase.offset.setTargetAtTime(0, now, 0.02);
    this.duckScale.gain.setTargetAtTime(0, now, 0.01);
    ...
    const fb = Math.min(0.95, (this.repeatsParam / 100) * 0.95);
    this.feedbackL.gain.setTargetAtTime(fb, now, 0.01); this.feedbackR.gain.setTargetAtTime(fb, now, 0.01);
    ...
    switch(type){
      ...
      case 'reverse':
        this._patchReverse();
        if (this.reverseNode) {
          const win = 100 + (this.value1 / 100) * 1400;
          this.reverseNode.parameters.get('windowMs').setTargetAtTime(win, now, 0.02);
          const mx = (this.value2 ?? 100) / 100;
          this.reverseNode.parameters.get('mix').setTargetAtTime(mx, now, 0.02);
        }
        break;
      ...
      case 'duck':
        this.duckBase.offset.setTargetAtTime(1.0, now, 0.02);
        if (!this._duckConnected) {
          this.duckBase.connect(this.delayMix.gain);
          this.duckDetLP.connect(this.duckScale);
          this.duckScale.connect(this.delayMix.gain);
          this._duckConnected = true;
        }
    ...
    if (type === 'lofi' || type === 'dbucket') this.updateMachineSpecificParams();
  }

  // Reverse envelope helper (legacy swell; real reverse uses reverseNode)
  ...
  updateParameter(parameter, value){ 
    ...
    switch(parameter){
      ...
      case 'repeats':{ this.repeatsParam = Math.max(0, Math.min(100, Number(value) || 0)); const fb=Math.min(0.95,(this.repeatsParam/100)*0.95); this.feedbackL.gain.setTargetAtTime(fb, now, 0.01); this.feedbackR.gain.setTargetAtTime(fb, now, 0.01); break; }
      ...
      case 'grit': this.gritAmount = value; this.gritL.curve = this.makeGritCurve(value); this.gritR.curve = this.makeGritCurve(value); break;
  ...
  updateMachineSpecificParams(){ const now=this.audioContext.currentTime;
    switch(this.currentType){
      ...
      case 'duck': {
        const amt = (this.value1 ?? 50) / 100;
        this.duckScale.gain.setTargetAtTime(-0.9 * amt, now, 0.02);
        break;
      }
      case 'reverse': {
        if (!this.reverseNode) break;
        const win = 100 + (this.value1 / 100) * 1400;
        this.reverseNode.parameters.get('windowMs').setTargetAtTime(win, now, 0.02);
        const mx = (this.value2 ?? 100) / 100;
        this.reverseNode.parameters.get('mix').setTargetAtTime(mx, now, 0.02);
      ...
      case 'dbucket': {
        if (this.bitcrushNode) {
          const bits = 4 + (this.value1 / 100) * 12;
          const ds = 1 + Math.floor((this.value2 / 100) * 15);
          const mx = (this.value3 ?? 100) / 100;
          this.bitcrushNode.parameters.get('bitDepth').setTargetAtTime(bits, now, 0.02);
          this.bitcrushNode.parameters.get('downsample').setTargetAtTime(ds, now, 0.02);
          this.bitcrushNode.parameters.get('mix').setTargetAtTime(mx, now, 0.02);
        }
        break;
      }
  ...
  getParameters() {
    const delayMs = this.delayL.delayTime.value * 1000;
    return {
      type: this.currentType,
      time: ((this.delayL.delayTime.value - 0.04) / 4.96) * 100,
      timeMs: Math.round(delayMs),
      repeats: this.repeatsParam ?? (this.feedbackL.gain.value / 0.95) * 100,
      filter: ((this.lpL.frequency.value - 500) / 11500) * 100,
      grit: this.gritAmount ?? 0,
  ...
  disconnect() {
    ...
    try {
      ...
      this.duckBase?.disconnect(); this.duckScale?.disconnect();
      if (this.iceUpL) this.iceUpL.disconnect();
      if (this.iceUpR) this.iceUpR.disconnect();
      if (this.reverseNode) this.reverseNode.disconnect();
      if (this.bitcrushNode) this.bitcrushNode.disconnect();
      this.postFxIn?.disconnect(); this.postFxOut?.disconnect();
